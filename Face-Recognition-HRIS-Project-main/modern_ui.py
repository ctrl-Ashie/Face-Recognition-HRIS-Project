...
class ModernButton(tk.Frame):
    def __init__(self, parent, text, command, bg=None, fg=None, width=140, height=38, corner_radius=8, **kwargs):
        ...
        try:
            ...
            parent_bg = ModernStyles.FORM_BG
    ...
    def config(self, **kwargs):
        ...
        # Pass any remaining kwargs to the frame
        frame_kwargs = {k: v for k, v in kwargs.items() if k not in ["state", "text"]}
        if frame_kwargs:
...
class ModernNavButton(tk.Frame):
    def __init__(self, parent, text, command=None, is_active=False, **kwargs):
        ...
        try:
            self._bg_idle = parent.cget("bg")
        except tk.TclError:
            self._bg_idle = ModernStyles.FORM_BG
            
        self.config(bg=self._bg_idle)
        self._state = state
        ...
        self._label = tk.Label(
            ...
            bg=ModernStyles.ACCENT_COLOR if is_active else self._bg_idle,
    ...
    def _on_enter(self):
        if not self.is_active and self.command and self._state != "disabled":
            hover_bg = self._lighten_color(ModernStyles.ACCENT_COLOR, 1.3)
            self._label.config(bg=hover_bg)
    
    def _on_leave(self):
        if not self.is_active and self._state != "disabled":
            self._label.config(bg=self._bg_idle)
    ...
    def set_active(self, active):
        ...
        self._label.config(
            bg=ModernStyles.ACCENT_COLOR if active else self._bg_idle,
    ...
    def config(self, **kwargs):
        ...
        if "bg" in kwargs:
            self._bg_idle = kwargs["bg"]
            if not self.is_active:
                self._label.config(bg=self._bg_idle)
            super().config(bg=self._bg_idle)
        if "text" in kwargs:
            self._label.config(text=kwargs["text"])
            
        frame_kwargs = {k: v for k, v in kwargs.items() if k not in ["state", "text", "bg"]}
        if frame_kwargs:
            super().config(**frame_kwargs)
...
def apply_modern_style():
    ...
    style.map("Treeview",
              background=[("selected", ModernStyles.ACCENT_COLOR)],
